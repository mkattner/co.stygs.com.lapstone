...
public class JavaScriptCompressor {
    ...
    public void compress(Writer out, Writer mungemap, int linebreak, boolean munge, boolean verbose, boolean preserveAllSemiColons, boolean disableOptimizations, boolean preserveUnknownHints)
	    throws IOException {
		...
		if (token.getValue().equals("app") && getToken(0).getType() == Token.DOT) {
			...
			if ((token = consumeToken()).getValue().equals("debug")) {
			    ...
			    if (token.getValue().equals("trace") || token.getValue().equals("debug") || token.getValue().equals("todo") || token.getValue().equals("info")
				    ...
				    || token.getValue().equals("validate") || token.getValue().equals("flat")) {
