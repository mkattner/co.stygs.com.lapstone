...
public class JavaScriptCompressor {
	...
	static {
		...
		// literals.put(new Integer(Token.APP_DEBUG), "app");
				...
				if (token.getType() == Token.ADD) {
					...
					if (prevToken.getType() == Token.STRING && nextToken.getType() == Token.STRING) {
						if (i < length - 2) {
							...
							if (nextNextToken.getType() == Token.DOT || nextNextToken.getType() == Token.LB) {
								i += 3;
								continue;
							}
						}
	 ...
	 * matches this pattern, it means it IS a valid identifier name. If a string
	 * doesn't match it, it does not necessarily mean it is not a valid identifier
	 * name.
	 */
	...
	private static void optimizeObjectMemberAccess(ArrayList tokens) {
		...
		for (i = 0, length = tokens.size(); i < length; i++) {
			...
			if (((JavaScriptToken) tokens.get(i)).getType() == Token.LB && i > 0 && i < length - 2 && ((JavaScriptToken) tokens.get(i - 1)).getType() == Token.NAME
					&& ((JavaScriptToken) tokens.get(i + 1)).getType() == Token.STRING && ((JavaScriptToken) tokens.get(i + 2)).getType() == Token.RB) {
	...
	public void compress(Writer out, int linebreak, boolean munge, boolean verbose, boolean preserveAllSemiColons, boolean disableOptimizations) throws IOException {
		compress(out, null, linebreak, munge, verbose, preserveAllSemiColons, disableOptimizations, false);
	}

	public void compress(Writer out, Writer mungemap, int linebreak, boolean munge, boolean verbose, boolean preserveAllSemiColons, boolean disableOptimizations,
			boolean preserveUnknownHints) throws IOException {
		...
		System.out.println("buildSymbolTree");
		buildSymbolTree();
		// DO NOT TOUCH this.tokens BETWEEN THESE TWO PHASES (BECAUSE OF
		// this.indexedScopes)
		mungeSymboltree();
		System.out.println("printSymbolTree");
	 ...
	 * scope or in any scope above it. Returns null if this symbol does not have a
	...
	/*
	 * If either 'eval' or 'with' is used in a local scope, we must make sure that
	 * all containing local scopes don't get munged. Otherwise, the obfuscation
	 * would potentially introduce bugs.
	 */
	...
	private StringBuffer printSymbolTree(int linebreakpos, boolean preserveAllSemiColons) throws IOException {
		...
		while (offset < length) {
			...
			case Token.NAME:
				String output = "";
				if (token.getValue().equals("app") && getToken(0).getType() == Token.DOT) {
					output += token.getValue();

					if ((token = consumeToken()).getType() == Token.DOT) {
						output += token.getValue();

						if ((token = consumeToken()).getValue().equals("debug")) {
							output += token.getValue();
							...
							if (token.getValue().equals("trace")
								...
								if ((token = consumeToken()).getType() == Token.LP) {
									output += token.getValue();
									nesting++;
								}

								while (!((token = consumeToken()).getType() == Token.RP && nesting == 1)) {

									if (token.getType() == Token.LP)
										nesting++;
									else if (token.getType() == Token.RP)
										nesting--;
									output += token.getValue();
								}
								...
								if (getToken(0).getType() == Token.SEMI) {
									token = consumeToken();
									output += token.getValue();
								}
