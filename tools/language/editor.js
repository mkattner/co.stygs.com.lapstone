var editor = {
  ...
  loadLanguageFiles: function() {
    ...
    $.ajax({
      ...
      $.each(json.availableLanguages, function(index, languageName) {
        var url;
        url = "../../files/language/" + index + ".json";
        $.ajax({
          dataType: "json",
          url: url,
          async: false
        }).done(function(json) {
          editor.languageObject[index] = json;
        }).fail(function() {
  ...
  mergeAll: function(primaryLanguage) {
    ...
    if (primaryLanguage != undefined) $.extend(true, editor.mergedLanguageObject, editor.languageObject[primaryLanguage]);
  }
}
...
$(document).ready(function() {
  ...
  $.each(editor.languageObject, function(languageId, languageObject) {
    ...
    divOperation.append('<a href="#" class="reset">Reset</a>');
    divOperation.append('<a href="#" class="deploy">Deploy</a>');
    divOperation.append('<a href="#" class="export">Export</a>');
  ...
  $.each(editor.mergedLanguageObject, function(contextId, contextObject) {
    ...
    if (typeof contextObject === 'object') {
      ...
      $.each(contextObject, function(translationId, translationValue) {
        ...
        divTranslation = $('<div class="divTranslation row" data-contextid="' + contextId + '" data-translation-id="' + translationId + '">');
        ...
        $.each(editor.languageObject, function(languageId, languageObject) {
          ...
          var inputTranslation = $('<input type="text" id="' + editor.inputId + '" class="translation" tabindex="' + editor.inputId + '" data-contextid="' + contextId + '" data-languageid="' + languageId + '" data-translationid="' + translationId + '"></input>');
          editor.inputId++;
...
$(document).on('keydown', 'input, textarea', function(e) {
  ...
  if ((e.ctrlKey || e.metaKey) && (String.fromCharCode(e.which) === '1')) {
    ...
    $('textarea[data-id=' + $(this).attr('id') + ']').focus();
    $('textarea[data-id=' + $(this).attr('id') + ']').attr('tabindex', $(this).attr('id'));
  } 
...
$(document).on('click', '.reset', function(event) {
  ...
  if (confirm("Do you realy want to reset all changes for this language?  It cant't be undone.\n\nLanguage: " + languageId)) {
    $('#divEditor').find('input[data-languageid=' + languageId + ']').each(function() {
...
$(document).on('click', '.deploy', function(event) {
  var languageId, exportObject = {};
  languageId = $(this).parent().attr("data-languageid");
  if (confirm("Do you realy want to deploy all your changes to the app? It cant't be undone.")) {
    $('#divEditor').find('input[data-languageid=' + languageId + ']').each(function() {
