...
// >>group: Core
// >>description: A handler for css transition & animation end events to ensure
		...
		if ( ( $.support.cssTransitions && animationType === "transition" ) ||
			...
			if ( fallbackTime === undefined ) {
				...
				if ( this.context !== document ) {
					...
					// Parse the durration since its in second multiple by 1000
					// for milliseconds
					// Multiply by 3 to make sure we give the animation plenty
					// of time.
					duration = parseFloat(
...
$.extend( $.mobile, {
	...
	// Retrieve an attribute from an element and perform some massaging of the
	// value
	...
	// Find the closest javascript page element to gather settings data jsperf
	// test
	// http://jsperf.com/single-complex-selector-vs-many-complex-selectors/edit
	// possibly naive, but it shows that the parsing overhead for *just* the
	// page selector vs
	// the page and dialog selector is negligable. This could probably be speed
	// up by
	// doing a similar parent node traversal to the one found in the inherited
	// theme code above
...
return $.extend( $.mobile, {
	...
	// Automatically handle clicks and form submissions through Ajax, when
	// same-domain
	ajaxEnabled: true,
...
// Subtract the height of external toolbars from the page height, if the page
// does not have
// internal toolbars of the same type. We take care to use the widget options if
// we find a
...
var compensateToolbars = function( page, desiredHeight ) {
	var pageParent = page.parent(),
		...
		// We use this function to filter fixed toolbars with option
		// updatePagePadding set to
		// true (which is the default) from our height subtraction, because
		// fixed toolbars with
		// option updatePagePadding set to true compensate for their presence by
		// adding padding
		// to the active page. We want to avoid double-counting by also
		// subtracting their
	...
	// If we have no internal headers, but we do have external headers, then
	// their height
	...
	// If we have no internal footers, but we do have external footers, then
	// their height
	// reduces the page height
...
$.extend( $.mobile, {
	...
	focusPage: function( page ) {
		...
		// If we do not find an element with the "autofocus" attribute, look for
		// the page title
	...
	// Scroll page vertically: scroll to 0 to hide iOS address bar, or pass a Y
	// value
	...
	getScreenHeight: function() {
		// Native innerHeight returns more accurate value for this across
		// platforms,
		// jQuery version is here as a normalized fallback for platforms like
		// Symbian
	...
	resetActivePageHeight: function( height ) {
		...
		// Set the minimum height only if the height as determined by CSS is
		// insufficient
	...
	loading: function() {
		// If this is the first call to this function, instantiate a loader
		// widget
...
$.fn.extend( {
	...
	// to return the html encoded version of the text in all cases. (thus the
	// name)
...
var bb = window.blackberry && !propExists( "-webkit-transform" ); // only used
																	// to rule
																	// out box
																	// shadow,
																	// as it's
...
function transform3dTest() {
	var mqProp = "transform-3d",
		// Because the `translate3d` test below throws false positives in
		// Android:
...
function fixedPosition() {
	...
	if (
			// iOS 4.3 and older : Platform is iPhone/Pad/Touch and Webkit
			// version is less than 534 (ios5)
			( ( platform.indexOf( "iPhone" ) > -1 || platform.indexOf( "iPad" ) > -1 || platform.indexOf( "iPod" ) > -1 ) && wkversion && wkversion < 534 ) ||
			// Opera Mini
			( w.operamini && ( {} ).toString.call( w.operamini ) === "[object OperaMini]" ) ||
			( operammobilematch && omversion < 7458 ) ||
			// Android lte 2.1: Platform is Android and Webkit version is less
			// than 533 (Android 2.2)
...
// $.mobile.ajaxBlacklist is used to override ajaxEnabled on platforms that have
// known conflicts with hash history updates (BB5, Symbian)
// or that generally work better browsing in regular http for full page
// refreshes (Opera Mini)
// Note: This detection below is used as a last resort.
// We recommend only using these detection methods when all other more
// reliable/forward-looking approaches are not possible
nokiaLTE7_3 = ( function() {

	var ua = window.navigator.userAgent;

	// The following is an attempt to match Nokia browsers that are running
	// Symbian/s60, with webkit, version 7.3 or older
...
// Lastly, this workaround is the only way we've found so far to get pre 7.3
// Symbian webkit devices
// to render the stylesheets when they're referenced before this script, as we'd
// recommend doing.
...
//
// The current version exposes the following virtual events to jQuery bind
// methods:
...
function createVirtualEvent( event, eventType ) {
	...
	// addresses separation of $.event.props in to $.event.mouseHook.props and
	// Issue 3280
...
function getSpecialEventObject( eventType ) {
	...
	return {
		setup: function( /* data, namespace */ ) {
			...
			// for elements unless they actually have handlers registered on
			// them.
			// To get around this, we register dummy handlers on the elements.

			$( this ).bind( realType, dummyMouseHandler );

			// For now, if event capture is not supported, we rely on mouse
			// handlers.
			if ( eventCaptureSupported ) {
				...
				if ( activeDocHandlers[ "touchstart" ] === 1 ) {
					$document.bind( "touchstart", handleTouchStart )
						...
						// On touch platforms, touching the screen and then
						// dragging your finger
						// causes the window content to scroll after some
						// distance threshold is
						// exceeded. On these platforms, a scroll prevents a
						// click event from being
						// dispatched, and on some platforms, even the touchend
						// is suppressed. To
						// mimic the suppression of the click event, we need to
						// watch for a scroll
						// event. Unfortunately, some platforms like iOS don't
						// dispatch scroll
						// events until *AFTER* the user lifts their finger
						// (touchend). This means
						// we need to watch both scroll and touchmove events to
						// figure out whether
						// or not a scroll happenens before the touchend event
						// is fired.
...
if ( eventCaptureSupported ) {
	document.addEventListener( "click", function( e ) {
		...
		if ( cnt ) {
			...
			// synthesized
			// from a touch event seems to be implementation specific. For
			// example,
			// some browsers will fire mouse/click events for a link that is
			// near
			// a touch event, even though the target of the touchstart/touchend
			// event
			// says the user touched outside the link. Also, it seems that with
			// most
			// browsers, the target of the mouse/click event is not calculated
			// until the
			// time it is dispatched, so if you replace an element that you
			// touched
			// with another element, the target of the mouse/click will be the
			// new
			// element underneath that point.
			//
			// Aside from proximity, we also check to see if the target and any
			// of its ancestors were the ones that blocked a click. This is
			// necessary
			// because of the strange mouse/click target calculation done in the
			// Android 2.1 browser, where if you click on an element, and there
			// is a
			// mouse/click handler on one of its ancestors, the target will be
			// the
			// innermost child of the touched element, even if that child is no
			// where
			...
			while ( ele ) {
				for ( i = 0; i < cnt; i++ ) {
					...
					if ( ( ele === target && Math.abs( o.x - x ) < threshold && Math.abs( o.y - y ) < threshold ) ||
						...
						// XXX: We may want to consider removing matches from
						// the block list
...
// >>label: Touch
// >>group: Events
// >>description: Touch events including: touchstart, touchmove, touchend, tap,
// taphold, swipe, swipeleft, swiperight
			...
			context.move = function( event ) {
				...
				if ( !emitted ) {
					...
					if ( emitted ) {
						...
						// Reset the context to make way for the next swipe
						// event
...
/*
 * ! jQuery Mobile Throttled Resize
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
// >>group: Events
// >>description: Fires a resize event with a slight delay to prevent excessive
// callback invocation
...
// >>group: Events
// >>description: Provides a wrapper around the inconsistent browser
// implementations of orientationchange
...
// It seems that some device/browser vendors use window.orientation values 0 and
// 180 to
// denote the "default" orientation. For iOS devices, and most other
// smart-phones tested,
// the default orientation is always "portrait", but in some Android and RIM
// based tablets,
// the default orientation is "landscape". The following code attempts to use
// the window
// dimensions to figure out what the current orientation is, and then makes
// adjustments
// to the to the portrait_map if necessary, so that we can properly decode the
// window.orientation value whenever get_orientation() is called.
//
// Note that we used to use a media query to figure out what the orientation the
// browser
// thinks it is in:
//
// initial_orientation_is_landscape = $.mobile.media("all and (orientation:
// landscape)");
//
// but there was an iPhone/iPod Touch bug beginning with iOS 4.2, up through iOS
// 5.1,
// where the browser *ALWAYS* applied the landscape media query. This bug does
// not
// happen on iPad.

if ( $.support.orientation ) {

	// Check the window width and height to figure out what the current
	// orientation
	// of the device is at this moment. Note that we've initialized the portrait
	// map
	// values to 0 and 180, *AND* we purposely check for landscape so that if we
	// guess
	// wrong, , we default to the assumption that portrait is the default
	// orientation.
	// We use a threshold check below because on some platforms like iOS, the
	// iPhone
	// form-factor can report a larger width than height if the user turns on
	// the
	// developer console. The actual threshold value is somewhat arbitrary, we
	// just
	// need to make sure it is large enough to exclude the developer console
	// case.
	...
	// If the initial orientation is landscape, but window.orientation reports 0
	// or 180, *OR*
	// if the initial orientation is portrait, but window.orientation reports 90
	// or -90, we
	// need to flip our portrait_map values because landscape is the default
	// orientation for
	// this device/browser.
...
$.event.special.orientationchange.orientation = get_orientation = function() {
	...
	// the actual screen resize takes place before or after the orientation
	// change event
	// has been fired depending on implementation (eg android 2.3 is before,
	// iphone after).
	// More testing is required to determine if a more reliable method of
	// determining the new screensize
	// is possible when orientationchange is fired. (eg, use media queries +
	// element + opacity)
	if ( $.support.orientation ) {
...
$.extend( $.enhance, {
	...
	// Check if the enhancer has already been defined if it has copy its hooks
	// if not
...
$.mobile.path = path = {
	...
	// This scary looking regular expression parses an absolute URL or its
	// relative
	// variants (protocol, site, document, query, and hash), into the various
	// components (protocol, host, path, query, fragment, etc that make up the
	// URL as well as some other commonly used sub-parts. When used with
	// RegExp.exec()
	// or String.match, it parses the URL into a results array that looks like
	// this:
	//
	// [0]:
	...
	// replaced with a call to this method so that it can be dealt with properly
	// here
	getLocation: function( url ) {
		var parsedUrl = this.parseUrl( url || location.href ),
			uri = url ? parsedUrl : location,

			// Make sure to parse the url or the location object for the hash
			// because using
			// location.hash is autodecoded in firefox, the rest of the url
			// should be from
			// the object (location unless we're testing) to avoid the inclusion
			// of the
			// authority
			hash = parsedUrl.hash;
		...
		return uri.protocol +
			...
			// The pathname must start with a slash if there's a protocol,
			// because you
			// can't have a protocol followed by a relative path. Also, it's
			// impossible to
			// calculate absolute URLs from relative ones if the absolute one
			// doesn't have
			// a leading "/".
	...
	parseUrl: function( url ) {
		...
		// like all other browsers do, so we normalize everything so its
		// consistent
	...
	// can handle all relative variants (protocol, site, document, query,
	// fragment).
	...
	convertUrlToDataUrl: function( absUrl ) {
		...
		if ( path.isEmbeddedPage( u ) ) {
			// For embedded pages, remove the dialog hash key as in
			// getFilePath(),
			// and remove otherwise the Data Url won't match the id of the
			// embedded Page.
			result = u.hash
	...
	// return a url path with the window's location protocol/hostname/pathname
	// removed
	clean: function( url ) {
	...
	// check whether a url is referencing the same domain, or an external domain
	...
	isEmbeddedPage: function( url ) {
		...
		// both the this.documentUrl and the documentBase. The main reason for
		// this
	...
	squash: function( url, resolutionUrl ) {
		...
		// If the url is a full url with a hash check if the parsed hash is a
		// path
		...
		if ( isPath ) {
			...
			// reconstruct each of the pieces with the new search string and
			// hash
	...
	// return the substring of a filepath before the dialogHashKey, for making a
	// server
	// request
...
// >>group: Navigation
// >>description: Manages a stack of history entries. Used exclusively by the
// Navigation Manager
...
$.extend( $.mobile.History.prototype, {
	...
	closest: function( url, id ) {
		...
		// First, we check whether we've found an entry by id. If so, we're
		// done.
		if ( closest !== undefined ) {
			return closest;
		}

		// Failing that take the slice of the history stack before the current
		// index and search
		// for a url match. If one is found, we'll avoid avoid looking through
		// forward history
		// NOTE the preference for backward history movement is driven by the
		// fact that
		// most mobile browsers only have a dedicated back button, and users
		// rarely use
		...
		// of the slice must then be added to the result to get the element
		// index
...
// >>group: Navigation
// >>description: Manages URL history and information in conjunction with the
// navigate event
...
$.extend( $.mobile.Navigator.prototype, {
	...
	squash: function( url, data ) {
		...
		// make sure to provide this information when it isn't explicitly set in
		// the
		...
		// is not fired.
		//
		// Check is to make sure that jqm doesn't throw type errors in
		// environments where
		...
		// If we haven't yet received the initial popstate, we need to update
		// the reference
	...
	hash: function( url, href ) {
		...
		if ( loc.pathname + loc.search === parsed.pathname + parsed.search ) {
			// If the pathname and search of the passed url is identical to the
			// current loc
			...
			// If the passed url is a path, make it domain relative and remove
			// any trailing hash
	...
	go: function( url, data, noEvents ) {
		...
		// Get the url as it would look squashed on to the current resolution
		// url
		href = path.squash( url );
		...
		// if there will be no hashchange fired (since that won't reset the
		// value)
		...
		// IMPORTANT in the case where popstate is supported the event will be
		// triggered
		// directly, stopping further execution - ie, interupting the flow of
		// this
		// method call to fire bindings at this expression. Below the navigate
		// method
		...
		//
		// if the url is a path we want to preserve the query params that are
		// available on
		// the current url.
		this.preventHashAssignPopState = true;
		window.location.hash = hash;

		// If popstate is enabled and the browser triggers `popstate` events
		// when the hash
		// is set (this often happens immediately in browsers like Chrome), then
		// the
		// this flag will be set to false already. If it's a browser that does
		// not trigger
		// a `popstate` on hash assignement or `replaceState` then we need avoid
		// the branch
		// that swallows the event created by the popstate generated by the hash
		// assignment
		// At the time of this writing this happens with Opera 12 and some
		// version of IE
	...
	// when execution of the `$.navigate` method stops at window.location.hash =
	// url;
	// and completely prevent them from propagating. The popstate event will
	// then be
	// retriggered after execution resumes
	//
	// TODO grab the original event here and use it for the synthetic event in
	// the
	...
	popstate: function( event ) {
		...
		// If this is the popstate triggered by the actual alteration of the
		// hash
		...
		// if this is the popstate triggered after the `replaceState` call in
		// the go
		// method, then simply ignore it. The history entry has already been
		// captured
		...
		// TODO: Do we really need all these conditions? Comparing location
		// hrefs
		...
		// account for direct manipulation of the hash. That is, we will receive
		// a popstate
		// when the hash is changed by assignment, and it won't have a state
		// associated. We
		// then need to squash the hash. See below for handling of hash
		// assignment that
		...
		// If all else fails this is a popstate that comes from the back or
		// forward buttons
		// make sure to set the state of our history stack properly, and record
		// the directionality
		this.history.direct( {
			id: ( event.originalEvent.state || {} ).id,
			url: ( event.originalEvent.state || {} ).url || hash,

			// When the url is either forward or backward in history include the
			// entry
			// as data on the event object for merging as data in the navigate
			// event
			present: function( historyEntry, direction ) {
				// make sure to create a new object to pass down as the navigate
				// event data
	...
	// NOTE must bind before `navigate` special event hashchange binding
	// otherwise the
	// navigation data won't be attached to the hashchange event in time for
	// those
	// bindings to attach it to the `navigate` special event
	// TODO add a check here that `hashchange.navigate` is bound already
	// otherwise it's
	// broken (exception?)
	hashchange: function( event ) {
		var history, hash;

		// If hashchange listening is explicitly disabled or pushstate is
		// supported
		...
		// On occasion explicitly want to prevent the next hash from propagating
		// because we only
		...
		this.history.direct( {
			...
			// When we don't find a hash in our history clearly we're aiming to
			// go there
			// record the entry as new for future traversal
			//
			// NOTE it's not entirely clear that this is the right thing to do
			// given that we
			// can't know the users intention. It might be better to explicitly
			// _not_
			// support location.hash assignment in preference to $.navigate
			// calls
			// TODO first arg to add should be the href, but it causes issues in
			// identifying
			// embedded pages
			missing: function() {
				history.add( hash, {
...
// >>group: Navigation
// >>description: A wrapper for the primary Navigator and History objects in
// jQuery Mobile
...
// TODO consider queueing navigation activity until previous activities have
// completed
...
// expose the history on the navigate method in anticipation of full integration
// with
// existing navigation functionalty that is tightly coupled to the history
// information
...
var base,
	...
	getDynamicEnabled = function() {
		...
		// If a value has been set at the old, deprecated location, we return
		// that value.
		// Otherwise we return the value from the new location. We check
		// explicitly for
		// undefined because true and false are both valid values for
		// dynamicBaseEnabled.
...
base = {
	...
	// Make sure base element is defined, for use in routing asset urls that are
	// referenced
	...
	set: function( href ) {
		...
		// manually.
		// Note: Our method of ascertaining whether the user wants to manager
		// their url base
		// manually is DEPRECATED as of 1.5.0 and will be removed in 1.6.0. As
		// of 1.6.0 the
		// flag base.dynamicBaseEnabled will be checked, so the function
		// getDynamicEnabled()
		// will be removed.
...
/*
 * ! jQuery Mobile Enhancer Backcompat@VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
 * license. http://jquery.org/license
 */
...
$.widget( "mobile.page", {
	...
	bindRemove: function( callback ) {
		...
		// When dom caching is not enabled or the page is embedded bind to
		// remove the page on hide
		if ( !page.data( "mobile-page" ).options.domCache &&
			...
			this._on( this.document, {
				pagecontainerhide: callback || function( e, data ) {
					...
					// Check if this is a same page transition and if so don't
					// remove the page
					if ( !data.samePage ) {
...
// >>group: Navigation
// >>description: Widget to create page container which manages pages and
// transitions
...
// These variables make all page containers use the same queue and only navigate
// one at a time
...
$.widget( "mobile.pagecontainer", {
	...
	options: {
		...
		changeOptions: {
			...
			// Use changeUrl instead, changeHash is deprecated and will be
			// removed in 1.6
			changeHash: true,
			fromHashChange: false,
			duplicateCachedPage: undefined,

			// loading message shows by default when pages are being fetched
			// during change()
			showLoadMsg: true,
			dataUrl: undefined,
			fromPage: undefined,
	...
	_create: function() {
		...
		// In the future this will be tracked to give easy access to the active
		// pagecontainer
		...
		this._on( this.window, {
			...
			// Disable a scroll setting when a hashchange has been fired, this
			// only works because
			// the recording of the scroll position is delayed for 100ms after
			// the browser might
	...
	_afterContentChange: function() {
		...
		// Remove any binding that previously existed on the get scroll which
		// may or may not be
	...
	// TODO active page should be managed by the container (ie, it should be a
	// property)
	getActivePage: function() {
		return this.activePage;
	},

	// TODO the first page should be a property set during _create using the
	// logic
	...
	// The options by which a given page was reached are stored in the history
	// entry for that
	// page. When this function is called, history is already at the new entry.
	// So, when moving
	// back, this means we need to consult the old entry and reverse the meaning
	// of the
	// options. Otherwise, if we're moving forward, we need to consult the
	// options for the
	// current entry.
	...
	_handleDialog: function( changePageOptions, data ) {
		...
		// Note: The dialog widget is deprecated as of 1.4.0 and will be removed
		// in 1.5.0.
		// Thus, as of 1.5.0 activeContent.data( "mobile-dialog" ) will always
		// evaluate to
		// falsy, so the second condition in the if-statement below can be
		// removed altogether.
	...
	_handleNavigate: function( url, data ) {
		...
		var to = $.mobile.path.stripHash( url ),
			...
			// Transition is false if it's the first page, undefined otherwise
			// (and may be
			// overridden by default)
			transition = history.stack.length === 0 ? "none" :
				this._optionFromHistory( data.direction, "transition" ),

			// Default options for the changPage calls made after examining the
			// current state of
			// the page and the hash, NOTE that the transition is derived from
	...
	_parse: function( html, fileUrl ) {
		...
		// TODO consider allowing customization of this method. It's very JQM
		// specific
		var page,
			all = $( "<div></div>" );
		...
		// TODO tagging a page with external to make sure that embedded pages
		// aren't
		// removed by the various page handling code is bad. Having page
		// handling code
	...
	// TODO it would be nice to split this up more but everything appears to be
	// "one off"
	// or require ordering such that other bits are sprinkled in between parts
	// that
	...
	_loadSuccess: function( absUrl, triggerData, settings, deferred ) {
		...
		return $.proxy( function( html, textStatus, xhr ) {
			...
			// Pre-parse html to check for a data-url, use it as the new
			// fileUrl, base path, etc
			var content,
			...
			// can be directed to
			// the correct url. loading into a temprorary element makes these
			...
			if ( pageElemRegex.test( html ) &&
				...
				// We specify that, if a data-url attribute is given on the page
				// div, its value
				// must be given non-URL-encoded. However, in this part of the
				// code, fileUrl is
				// assumed to be URL-encoded, so we URL-encode the retrieved
				// value here
	...
	load: function( url, options ) {
		...
		// the content. For
		// embedded content, it is just the id of the page. For content within
		// the same domain as
		// the document base, it is the site relative path. For cross-domain
		// content (PhoneGap
		...
		// If the content we are interested in is already in the DOM, and the
		// caller did not
		// indicate that we should force a reload of the file, we are done.
		// Resolve the deferrred
		// so that users can bind to .done on the promise
		if ( content.length && !settings.reload ) {
			this._enhance( content, settings.role );
			deferred.resolve( absUrl, settings, content );

			// If we are reloading the content make sure we update the base if
			// its not a prefetch
		...
		// Reset base to the default document base. Only reset if we are not
		// prefetching.
		if ( settings.prefetch === undefined ) {
			this._getBase().reset();
		}
	...
	_loadError: function( absUrl, triggerData, settings, deferred ) {
		return $.proxy( function( xhr, textStatus, errorThrown ) {
			...
			// Clean up internal pending operations like the loader and the
			// transition lock
			this._hideLoading();
	...
	_getTransitionHandler: function( transition ) {
		...
		// isn't one in our
		// transitionHandlers dictionary, use the default one. call the handler
		// immediately to
	...
	_triggerCssTransitionEvents: function( to, from, prefix ) {
		...
		// TODO decide if these events should in fact be triggered on the
		// container
		if ( from ) {

			// Check if this is a same page transition and tell the handler in
			// page
			if ( to[ 0 ] === from[ 0 ] ) {
				samePage = true;
			}
	...
	_loadUrl: function( to, triggerData, settings ) {
		...
		// eg, removing
		// ui-state, and removing query params from the hash this is so that
		// users who want to use
		// query params have access to them in the event bindings for the page
		// life cycle
		// See issue #5085
		settings.target = to;
		...
		settings.deferred.done( $.proxy( function( url, options, content ) {
			...
			// Store the original absolute url so that it can be provided to
			// events in the
	...
	_triggerPageBeforeChange: function( to, triggerData, settings ) {
		...
		if ( $.type( to ) === "string" ) {
			...
			// If the toPage is a jQuery object grab the absolute url stored in
			// the load()
	...
	change: function( to, options ) {
		...
		// We'll call
		// change() once we're done with the current transition to service the
		// request.
		...
		// We allow "pagebeforechange" observers to modify the to in the trigger
		// data to allow for
		// redirects. Make sure our to is updated. We also need to re-evaluate
		// whether it is a
		// string, because an object can also be replaced by a string
		to = triggerData.toPage;

		// If the caller passed us a url, call load() to make sure it is loaded
		// into the DOM.
		// We'll listen to the promise object it returns so we know when it is
		// done loading or if
		// an error ocurred.
		if ( $.type( to ) === "string" ) {
	...
	transition: function( toPage, triggerData, settings ) {
		...
		if ( isPageTransitioning ) {
			...
			// Make sure to only queue the to and settings values so the
			// arguments work with a call
			// to the change method
			pageTransitionQueue.unshift( [ toPage, settings ] );
			return;
		}

		// DEPRECATED - this call only, in favor of the before transition if the
		// page beforechange
		...
		// If the (content|page)beforetransition default is prevented return
		// early. Note, we have
		...
		// Set the isPageTransitioning flag to prevent any requests from
		// entering this method while
		// we are in the midst of loading a page or transitioning.
		isPageTransitioning = true;

		// If we are going to the first-page of the application, we need to make
		// sure
		// settings.dataUrl is set to the application document url. This allows
		// us to avoid
		// generating a document url with an id hash in the case where the
		// first-page of the
		...
		// The caller passed us a real page DOM element. Update our internal
		// state and then trigger
		// a transition to the page.
		fromPage = settings.fromPage;
		url = ( settings.dataUrl && $.mobile.path.convertUrlToDataUrl( settings.dataUrl ) ) ||
			toPage.jqmData( "url" );

		// The pageUrl var is usually the same as url, except when url is
		// obscured as a dialog url.
		...
		// are the same
		// element, but folks that generate content manually/dynamically and
		// reuse pages want to be
		// able to transition to the same page. To allow this, they will need to
		// change the default
		// value of allowSamePageTransition to true, *OR*, pass it in as an
		// option when they
		// manually call change(). It should be noted that our default
		// transition animations
		// assume that the formPage and toPage are different elements, so they
		// may behave
		// unexpectedly. It is up to the developer that turns on the
		// allowSamePageTransitiona
		// option to either turn off transition animations, or make sure that an
		// appropriate
		...
		// see if the page is
		// already within the urlHistory stack. If so, we'll assume the user hit
		// the forward/back
		...
		// We blur the focused element to cause the virtual keyboard to
		// disappear
		...
		// If we're displaying the page as a dialog, we don't want the url for
		// the dialog content
		// to be used in the hash. Instead, we want to append the dialogHashKey
		// to the url of the
		// current page.
		if ( isDialog && active ) {
			...
			// should be an empty
			// string. Moving the undefined -> empty string back into
			// urlHistory.addNew seemed
			// imprudent given undefined better represents the url state

			// If we are at a place in history that once belonged to a dialog,
			// reuse this state
			// without adding to urlHistory and without modifying the hash.
			// However, if a dialog is
			// already displayed at this point, and we're about to display
			// another dialog, then we
			// must add another hash and history entry on top so that one may
			...
			// Normally, we tack on a dialog hash key, but if this is the
			// location of a stale
			// dialog, we reuse the URL from the entry
			url = ( active.url || "" );

			// Account for absolute urls instead of just relative urls use as
			// hashes
		...
		cssTransitionDeferred.done( $.proxy( function( name, reverse, $to, $from, alreadyFocused ) {
			...
			// If there's a duplicateCachedPage, remove it from the DOM now that
			// it's hidden
...
$.extend( $.mobile.Transition.prototype, {
	...
	doneIn: function() {
		...
		// In some browsers (iOS5), 3D transitions block the ability to scroll
		// to the desired location during transition
		// This ensures we jump to that spot after the fact, if we aren't there
		// already.
	...
	scrollPage: function() {
		// By using scrollTo instead of silentScroll, we can keep things better
		// in order
		// Just to be precautios, disable scrollstart listening like
		// silentScroll would
		$.event.special.scrollstart.enabled = false;
		// if we are hiding the url bar or the page was previously scrolled
		// scroll to hide or return to position
	...
	transition: function( toScroll ) {
		// NOTE many of these could be calculated/recorded in the constructor,
		// it's my
		// opinion that binding them as late as possible has value with regards
		// to
		// better transitions with fewer bugs. Ie, it's not guaranteed that the
		// object will be created and transition will be run immediately after
		// as
		// it is today. So we wait until transition is invoked to gather the
		// following
		var none,
...
// >>group: Transitions
// >>description: Animated page change with concurrent transition style
// application
// >>demos: http://demos.jquerymobile.com/@VERSION/transitions/
...
// If transition is defined, check if css 3D transforms are supported, and if
// not, if a fallback is specified
...
// Set the getMaxScrollForTransition to default if no implementation was set by
// user
...
// >>label: Content Management
// >>group: Navigation
// >>description: Applies the AJAX navigation system to links and forms to
...
var domreadyDeferred = $.Deferred(),
	...
	// used to track last vclicked element to make sure its value is added to
	// form data
	$lastVClicked = null;
...
$.mobile._registerInternalEvents = function() {
	var getAjaxFormData = function( $form, calculateOnly ) {
		...
		if ( !url ) {
			...
			// NOTE: If the method is "get", we need to strip off the query
			// string
			// because it will get replaced with the new form data. See issue
			// #5710.
			if ( method === "get" ) {
		...
		if ( !calculateOnly ) {
			...
			if ( $lastVClicked && $lastVClicked[ 0 ].form === $form[ 0 ] ) {
				...
				if ( vclickedName ) {
					// Make sure the last clicked element is included in the
					// form
					$.each( formData, function( key, value ) {
						if ( value.name === vclickedName ) {
							// Unset vclickedName - we've found it in the
	...
	$.mobile.document.bind( "vclick", function( event ) {
		...
		// that when the virtual event is generated it will create the which
		// attr
		...
		// Try to find a target element to which the active class will be
		// applied
		if ( $.data( target, "ui-button" ) ) {
			// If the form will not be submitted via AJAX, do not add active
			// class
			...
			// TODO teach $.mobile.hijackable to operate on raw dom elements so
			// the
	...
	$.mobile.document.bind( "click", function( event ) {
		...
		var link = findClosestLink( event.target ),
			...
			// remove active link class if external (then it won't be there if
			// you come back)
		...
		// If a button was clicked, clean up the active class added by vclick
		// above
		if ( $.mobile.activeClickedLink &&
		...
		// protocol.
		// This is usually to allow the application to "phone home" and fetch
		// app specific
		// data. We normally let the browser handle external/cross-domain urls,
		// but if the
		// allowCrossDomainPages option is true, we will allow cross-domain
		// http/https
	...
	$.mobile.document.bind( "pageshow", function() {
		...
		// We need to wait for window.load to make sure that styles have already
		// been rendered,
...
$.widget( "mobile.loader", {
	...
	// NOTE that the $.mobile.loading* settings and params past the first are
	// deprecated
	...
	show: function( theme, msgText, textonly ) {
		...
		if ( $.type( theme ) === "object" ) {
			...
			// we prefer the
			// global option because we can't use undefined default prototype
			// options, then the
			// prototype option
			theme = theme || loadSettings.theme;
		}

		// Set the message text, prefer the param, then the settings object then
		// loading message
		...
		// Add the proper css given the options (theme, text, etc). Force text
		// visibility if the
		// second argument was supplied, or if the text was explicitly set in
		// the object args.
		...
		// If the pagecontainer widget has been defined we may use the
		// :mobile-pagecontainer and
		// attach to the element on which the pagecontainer widget has been
		// defined. If not, we
		// attach to the body.
		// TODO: Replace the selector below with $.mobile.pagecontainers[] once
		// #7947 lands
...
// trigger mobileinit event - useful hook for configuring $.mobile settings
// before they're used
$.mobile.document.trigger( "mobileinit" );

// support conditions
// if device support condition(s) aren't met, leave things as they are -> a
...
// override ajaxEnabled on platforms that have known conflicts with hash history
// updates
// or generally work better browsing in regular http for full page refreshes
// (BB5, Opera Mini)
...
// This is a fallback. If anything goes wrong (JS errors, etc), or events don't
// fire,
// this ensures the rendering class is removed after 5 seconds, so content is
...
$.extend( $.mobile, {
	...
	initializePage: function() {
		...
		// initialize navigation events now, after mobileinit has occurred and
		// the page container
		// has been created but before the rest of the library is alerted to
		// that fact
		...
		// the hash is not valid (contains more than one # or does not start
		// with #)
		// or there is no page with that hash, change to the first page in the
		// DOM
		...
		if ( !( $.mobile.hashListeningEnabled &&
			...
			if ( !$.event.special.navigate.isPushStateEnabled() ) {
				...
				// TODO figure out how to simplify this interaction with the
...
$( function() {
	...
	// note that this initial scroll won't hide the address bar. It's just for
	// the check.

	// hide iOS browser chrome on load if hideUrlBar is true this is to try and
	...
	// it should be 1 in most browsers, but android treats 1 as 0 (for hiding
	// addr bar)
	...
	if ( !$.support.cssPointerEvents ) {
		// IE and Opera don't support CSS pointer-events: none that we use to
		// disable link-based buttons
		// by adding the 'ui-disabled' class to them. Using a JavaScript
		// workaround for those browser.
...
/*
 * ! jQuery Mobile Flip Transition Fallback
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
// >>group: Transitions
// >>description: Flip transition fallback definition for non-3D supporting
// browsers
// >>demos: http://demos.jquerymobile.com/@VERSION/transitions/
...
/*
 * ! jQuery Mobile Flow Transition Fallback
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
// >>group: Transitions
// >>description: Flow transition fallback definition for non-3D supporting
// browsers
// >>demos: http://demos.jquerymobile.com/@VERSION/transitions/
...
/*
 * ! jQuery Mobile Pop Transition Fallback
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
// >>group: Transitions
// >>description: Pop transition fallback definition for non-3D supporting
// browsers
// >>demos: http://demos.jquerymobile.com/@VERSION/transitions/
...
// >>group: Transitions
// >>description: Slide transition fallback definition for non-3D supporting
// browsers
// >>demos: http://demos.jquerymobile.com/@VERSION/transitions/
...
/*
 * ! jQuery Mobile Slidedown Transition Fallback
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
// >>group: Transitions
// >>description: Slidedown transition fallback definition for non-3D supporting
// browsers
// >>demos: http://demos.jquerymobile.com/@VERSION/transitions/
...
/*
 * ! jQuery Mobile Slidefade Transition Fallback
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
// >>group: Transitions
// >>description: Slideback transition fallback definition for non-3D supporting
// browsers
// >>demos: http://demos.jquerymobile.com/@VERSION/transitions/
...
/*
 * ! jQuery Mobile Slideup Transition Fallback
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
// >>group: Transitions
// >>description: Slidep transition fallback definition for non-3D supporting
// browsers
// >>demos: http://demos.jquerymobile.com/@VERSION/transitions/
...
/*
 * ! jQuery Mobile Turn Transition Fallback
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
// >>group: Transitions
// >>description: Turn transition fallback definition for non-3D supporting
// browsers
// >>demos: http://demos.jquerymobile.com/@VERSION/transitions/
...
/*
 * ! jQuery Mobile Transition Visuals
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
/*
 * ! jQuery UI Unique ID master http://jqueryui.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
 * license. http://jquery.org/license
 */
...
/*
 * ! jQuery UI Accordion master http://jqueryui.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
 * license. http://jquery.org/license
 */
...
// >>group: Widgets
// >>description: Behavior mixin to mark first and last visible item with
// special classes.
...
$.widget( "mobile.collapsible", {
	...
	_getOptions: function( options ) {
		...
		for ( key in options ) {
			...
			// Retrieve the option value first from the options object passed in
			// and, if
			// null, from the parent accordion or, if that's null too, or if
			// there's no
	...
	_applyOptions: function( options ) {
		...
		// First and foremost we need to make sure the collapsible is in the
		// proper
	...
	_handleExpandCollapse: function( isCollapse ) {
		...
		if ( ui.icon ) {
			...
			// logic or cause same icon for expanded/collapsed state would
			// remove the ui-icon-class
...
return $.widget( "mobile.collapsibleset", $.extend( {
	...
	_init: function() {
		...
		// Because the corners are handled by the collapsible itself and the
...
/*
 * ! jQuery UI Controlgroup master http://jqueryui.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
 * license. http://jquery.org/license
 */
...
$.widget( "ui.controlgroup", $.ui.controlgroup, {
	...
	// This method is no longer necessary since controlgroup no longer has a
	// wrapper
	container: function() {
		return this.element;
	},
...
// >>group: Backcompat
// >>description: Synchronize deprecated style options and the value of the
// classes option.
	...
	$.mobile.widget = $.extend( {}, { backcompat: {
		...
		_setInitialOptions: function() {
			...
			if ( prop ) {
				...
				// If the classes option value has diverged from the default,
				// then its value takes
				// precedence, causing us to update all the style options to
				...
				// Otherwise we assume that we're dealing with legacy code and
				// look for style
				// option values which diverge from the defaults. If we find any
				// that diverge, we
...
/*
 * ! jQuery Mobile Controlgroup Backcompat
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
/*
 * ! jQuery Mobile Selectmenu Controlgroup Integration
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
return $.widget( "ui.controlgroup", $.ui.controlgroup, {
	...
	// The native element of an enhanced and disabled selectmenu widget fails
	// the :visible test.
	// This will cause controlgroup to ignore it in the calculation of the
	// corner classes. Thus, in
	// the case of the selectmenu, we need to transfer the controlgroup
	// information from the native
	// select element to its parent which is still visible.
	//
	// The selectmenu widget's wrapper needs to have the class ui-button-inline,
	// but only when the
	// selectmenu is oriented horizontally. Thus, we remove it here, and allow
	// the refresh() to
	...
	_initWidgets: function() {
		...
		this.childWidgets = this.childWidgets.map( function() {
			...
			if ( selectmenuWidget ) {
				...
				// Remove the class ui-button-inline. It may be re-added if this
				// controlgroup is
				// horizontal.
...
// >>label: Filterable
// >>group: Widgets
// >>description: Renders the children of an element filterable via a callback
// and a textinput
...
// TODO rename filterCallback/deprecate and default to the item itself as the
// first argument
...
return $.widget( "mobile.filterable", {
	...
	_filterItems: function( val ) {
		...
		// If nothing is hidden, then the decision whether to hide or show the
		// items
	...
	// TODO: When the input is not internal, do not even store it in
	// this._search
...
// >>label: Zoom Handling
// >>group: Utilities
// >>description: Utility methods for enabling and disabling user scaling (pinch
// zoom)
...
$.widget( "mobile.toolbar", {
	...
	// We only want this to run on non fixed toolbars so make it easy to
	// override
	...
	_updateBackButton: function() {
		...
		if ( this.options.addBackBtn &&
				...
				( this.page ?
					...
					// If the toolbar is internal the page's URL must differ
					// from the hash
					( this.page[ 0 ].getAttribute( "data-" + $.mobile.ns + "url" ) !==
						$.mobile.path.stripHash( location.hash ) ) :

					// Otherwise, if the toolbar is external there must be at
					// least one
		...
		// If we are not adding a back button, then remove the one present, if
		// any
	...
	_addHeadingClasses: function() {
		...
		this.headerElements
			...
			// Regardless of h element number in src, it becomes h1 for the
			// enhanced page
			.attr( {
				"role": "heading",
				"aria-level": "1"
...
return $.widget( "mobile.toolbar", $.mobile.toolbar, {
		...
		if ( this.options.position === "fixed" ) {
			...
			if ( o.fullscreen !== undefined ) {
				if ( o.fullscreen ) {
					...
					// If not fullscreen, add class to page to set top or bottom
					// padding
	...
	_bindPageEvents: function() {
		...
		// Page event bindings
		// Fixed toolbars require page zoom to be disabled, otherwise usability
		// issues crop up
		// This method is meant to disable zoom while a fixed-positioned toolbar
		// page is visible
		this._on( page, {
			"pagebeforeshow": "_handlePageBeforeShow",
	...
	// This will set the content element's top or bottom padding equal to the
	// toolbar's height
	updatePagePadding: function( tbPage ) {
		...
		// TbPage argument can be a Page object or an event, if coming from
		// throttled resize.
...
// >>label: Toolbars: Fixed
// >>group: Widgets
// >>description: Behavior for "fixed" headers and footers - be sure to also
// include the
// >> item 'Browser specific workarounds for "fixed" headers and footers' when
// supporting
// >> Android 2.x
// >>docs: http://api.jquerymobile.com/toolbar/
// >>demos: http://demos.jquerymobile.com/@VERSION/toolbar-fixed/
...
if ( $.mobileBackcompat !== false ) {
	...
	return $.widget( "mobile.toolbar", $.mobile.toolbar, {
		...
		_handlePageFocusinFocusout: function( event ) {
			...
			// This hides the toolbars on a keyboard pop to give more screen
			// room and prevent
			// ios bug which positions fixed toolbars in the middle of the
			// screen on pop if the
			// input is near the top or bottom of the screen addresses issues
			// #4410 Footer
			// navbar moves up when clicking on a textbox in an Android
			// environment and issue
			// #4113 Header and footer change their position after keyboard
			// popup - iOS and
			// issue #4410 Footer navbar moves up when clicking on a textbox in
			// an Android
			// environment
			if ( this.options.hideDuringFocus && screen.width < 1025 &&
				...
				// Fix for issue #4724 Moving through form in Mobile Safari with
				// "Next" and
				// "Previous" system controls causes fixed position, tap-toggle
				// false Header to
				// reveal itself isVisible instead of self._visible because the
				// focusin and
				// focusout events fire twice at the same time Also use a delay
				// for hiding the
				// toolbars because on Android native browser focusin is
				// direclty followed by a
				// focusout when a native selects opens and the other way around
				// when it closes.
				if ( event.type === "focusout" && !data.isVisible ) {
					data.isVisible = true;

					// Wait for the stack to unwind and see if we have jumped to
					// another input
					clearTimeout( data.delayHide );
					data.delayShow = this._delay( "show", 0 );
				} else if ( event.type === "focusin" && !!data.isVisible ) {

					// If we have jumped to another input clear the time out to
					// cancel the show
		...
		// Utility class for checking header and footer positions relative to
		// viewport
		...
		_bindScrollWorkaround: function() {
			...
			// Bind to scrollstop and check if the toolbars are correctly
			// positioned
			this._on( this.window, { scrollstop: function() {
					var viewportOffset = self._viewportOffset();

					// Check if the header is visible and if its in the right
					// place
		...
		// This addresses issue #4250 Persistent footer instability in v1.1 with
		// long select lists
		// in Android 2.3.3 and issue #3748 Android 2.x: Page transitions broken
		// when fixed toolbars
		// used the absolutely positioned thumbnail in a list view causes
		// problems with fixed
		// position buttons above in a nav bar setting the li's to
		// -webkit-transform:translate3d(0,0,0); solves this problem to avoid
		// potential issues in
		...
		// Addresses issues #4337 Fixed header problem after scrolling content
		// on iOS and Android
		// and device bugs project issue #1 Form elements can lose click hit
		// area in
		// position: fixed containers. This also addresses not on fixed toolbars
		// page in docs
		// adding 1px of padding to the bottom then removing it causes a
		// "redraw"
		// which positions the toolbars correctly (they will always be visually
		// correct)
		_triggerRedraw: function() {
			...
			// If the padding is reset with out a timeout the reposition will
			// not occure.
			// this is independent of JQM the browser seems to need the time to
			// react.
			setTimeout( function() {
...
/*
 * ! jQuery Mobile Fixed Toolbar Tap Toggle
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
// >>group: Forms
// >>description: Textarea elements automatically grow/shrink to accommodate
// their contents.
...
/*
 * ! jQuery UI Checkboxradio master http://jqueryui.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
 * license. http://jquery.org/license
 */
...
/*
 * ! jQuery UI Button master http://jqueryui.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
 * license. http://jquery.org/license
 */
...
/*
 * ! jQuery Mobile Button Backcompat
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
/*
 * ! jQuery Mobile Checkboxradio Backcompat
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
return $.widget( "mobile.textinput", $.mobile.textinput, {
	...
	_create: function() {
		...
		// We do nothing on startup if the options is off or if this is not a
		// wrapped input
...
// >>label: Form Reset
// >>group: Forms
// >>description: A behavioral mixin that forces a widget to react to a form
// reset
...
// >>label: Flip Switch
// >>group: Forms
// >>description: Consistent styling for native select menus. Tapping opens a
// native select menu.
...
return $.widget( "mobile.flipswitch", $.extend( {
	...
	_create: function() {
		...
		// On iOS we need to prevent default when the label is clicked,
		// otherwise it drops down
		// the native select menu. We nevertheless pass the click onto the
		// element like the
		// native code would.
	...
	_findLabels: function() {
		...
		if ( labelsList && labelsList.length ) {
			...
			if ( labelsList.length === 0 ) {
				...
				// NOTE: Windows Phone could not find the label through a
				// selector
				// filter works though.
...
/*
 * ! jQuery Mobile Flipswitch Backcompat
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
$.widget( "mobile.slider", $.extend( {
	...
	_create: function() {
		...
		// We have to instantiate a new function object for the unbind to work
		// properly
		// since the method itself is defined in the prototype (causing it to
		// unbind everything)
		...
		// Bind the handle event callbacks and set the context to the widget
		// instance
	...
	_preventDocumentDrag: function( event ) {
		...
		if ( this.dragging && !this.options.disabled ) {
			...
			// This.mouseMoved must be updated before refresh() because it will
			// be
...
$.widget( "mobile.rangeslider", $.extend( {
	...
	_slidestop: function( event ) {
		...
		// This stops dragging of the handle and brings the active track to the
		// front
	...
	_change: function( event ) {
		...
		if ( min > max && !this._sliderTarget ) {
			...
			// This makes it so clicks on the target on either extreme go to the
			// closest handle
		...
		// Fixes issue where when both _sliders are at min they cannot be
		// adjusted
		if ( min === max ) {
...
/*
 * ! jQuery Mobile Rangeslider Backcompat
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
var selectmenu = $.widget( "mobile.selectmenu", [ {
	...
	_create: function() {
		...
		// for these reasons, using the nativeMenu option results in a full
		// native select in Opera
	...
	build: function() {
		...
		// In many situations, iOS will zoom into the select upon tap, this
		// prevents that from
		// happening
		...
		this.button.children( "span" )
			...
			.append( ( function() {
				...
				if ( text ) {
					...
					// Set the contents to &nbsp; which we write as &#160; to be
					// XHTML compliant.
					// See gh-6699
					span.html( "&#160;" );
				}

				// Hide from assistive technologies, as otherwise this will
				// create redundant text
	...
	// Functions open and close preserved in native selects to simplify users
	// code when looping
	// over selects
	open: $.noop,
	close: $.noop,
...
// >>label: Listview
// >>group: Widgets
// >>description: Applies listview styling of various types (standard, numbered,
// split button, etc.)
...
return $.widget( "mobile.listview", $.extend( {
	...
	// We only handle the theme option through the theme extension. Theme
	// options concerning list
	// items such as splitTheme and dividerTheme have to be handled in
	// refresh().
	_themeElements: function() {
		return [ {
			element: this.element,
	...
	refresh: function( create ) {
		...
		// Check if a start attribute has been set while taking a value of 0
		// into account
		if ( ol && ( start || start === 0 ) ) {
			...
			if ( create || !listviewItemClassRegex.test( item[ 0 ].className ) ) {
				...
				if ( a.length && ( ( !buttonClassRegex.test( a[ 0 ].className ) && !isDivider ) ||
					...
					if ( a.length > 1 ) {
						...
						// Reduce to the first anchor, because only the first
						// gets the buttonClass
						a = a.first();
					} else if ( icon ) {
		...
		for ( dictionaryKey in itemClassDict ) {
			...
			// Split the dictionary key back into key classes and extra classes
			// and construct the
...
// >>group: Widgets
// >>description: Styles a page as a modal dialog with inset appearance and
// overlay background
// >>docs: http://api.jquerymobile.com/page/
	...
	_setOptions: function( options ) {
		...
		if ( options.overlayTheme !== undefined ) {
			if ( $.mobile.activePage[ 0 ] === this.element[ 0 ] ) {
				...
				// Needs the option value to already be set on this.options.
				// This is accomplished
				// by chaining up above, before handling the overlayTheme
				// change.
		...
		if ( this.dialog.button ) {
			...
			if ( "none" === location ) {
				...
				if ( text ) {
					...
					// Get rid of all text nodes without touching other node
					// types before updating
					// the button's text.
	...
	$.widget( "mobile.page", $.mobile.page, {
		...
		_create: function() {
			...
			if ( $.mobile.getAttribute( this.element[ 0 ], "role" ) === "dialog" ||
				...
				// The page container needs to distinguish a dialog widget from
				// a page styled
				// as a dialog. It does so by looking for the "mobile-dialog"
				// data item on the
				// page element. Since the dialog is no longer a widget, we need
				// to provide a
				// dummy hint
...
function fitSegmentInsideSegment( windowSize, segmentSize, offset, desired ) {
	...
	if ( windowSize < segmentSize ) {
		...
		// Else center it at the desired coordinate while keeping it completely
		// inside the window
...
$.widget( "mobile.popup", {
	...
	options: {
		...
		// NOTE Windows Phone 7 - 8.1 has a scroll position caching issue that
		// requires us to
	...
	// When the user depresses the mouse/finger on an element inside the popup
	// while the popup is
	...
	_create: function() {
		...
		// nav. We can't do it
		// in the option declarations because those are run before it is
		...
		if ( currentOptions.enhanced ) {
			...
			// We call _addClasses() even though as per the requirements for
			// enhanced: true, the
			// classes should already be added, because we want tracking for the
			// classes. This
			// should not result in reflows, because the class attribute will be
			// set to a value it
			// already has.
			this._addClasses();
		} else {
	...
	_enhance: function() {
		...
		// Using a wonderful golfing tradition we leave a token where the
		// element used to be so we
		// know where to put it back upon _destroy()
		ui.placeholder.html( "<!-- " + placeholder + " -->" ).insertBefore( this.element );

		// We detach the payload, add the classes, and then insert it into the
		// popup container
		this.element.detach();

		// This._ui needs to be fully established before we can all
		// this._addClasses(), because
		// this._addClasses() uses this._ui, but we don't want any of the UI
		// elements to be
	...
	_resizeScreen: function() {
		var screen = this._ui.screen,
			...
			// Subtracting 1 here is necessary for an obscure Android 4.0 bug
			// where the browser
		...
		if ( this._isOpen ) {
			if ( !this._expectResizeEvent() ) {
				if ( this._ui.container.hasClass( "ui-popup-hidden" ) ) {
					...
					// Effectively rapid-open the popup while leaving the screen
					// intact
	...
	_handleWindowResize: function( /* theEvent */ ) {
		if ( this._isOpen && this._ignoreResizeTo === 0 ) {
			if ( isOutOfSight( this._ui.container, getWindowCoordinates( this.window ) ) &&
				...
				// Effectively rapid-close the popup while leaving the screen
				// intact
	...
	_setTolerance: function( value ) {
		...
		if ( value !== undefined ) {
			...
			// The first value denotes top/bottom tolerance, and the second
			// value denotes
	...
	_calculateFinalLocation: function( desired, clampInfo ) {
		...
		// the window
		// tolerances. This will center wrt. the window if the popup is too
		// large.
		returnValue = {
	...
	_createPrerequisites: function( screenPrerequisite, containerPrerequisite, whenDone ) {
		...
		// functions which
		// call the callbacks passed in. The comparison between the local
		// variable and
		// self._prerequisites is necessary, because once a function has been
		// passed to
		// .animationComplete() it will be called next time an animation
		// completes, even if that's
		// not the animation whose end the function was supposed to catch (for
		// example, if an abort
		// happens during the opening animation, the .animationComplete handler
		// is not called for
		// that animation anymore, but the handler remains attached, so it is
		// called the next time
		// the popup is opened - making it stale. Comparing the local variable
		// prerequisites to the
		// widget-level variable self._prerequisites ensures that callbacks
		// triggered by a stale
	...
	_animate: function( args ) {
		...
		// NOTE Before removing the default animation of the screen this had an
		// animate callback
		// that would resolve the deferred. Now the deferred is resolved
		// immediately
	...
	// identified via desiredPosition.positionTo. Nevertheless, this function
	// ensures that its
	// return value always contains valid x and y coordinates by specifying the
	// center middle of
	...
	_openPrerequisitesComplete: function() {
		...
		// Check to see if currElement is not a child of the container. If it's
		// not, blur
	...
	_open: function( options ) {
		...
		// Count down to triggering "popupafteropen" - we have two
		// prerequisites:
		...
		if ( this.options.overlayTheme && androidBlacklist ) {
			...
			// suffers from an
			// issue where the popup overlay appears to be z-indexed above the
			// popup itself when
			// certain other styles exist on the same page -- namely, any
			// element set to
			// `position: fixed` and certain types of input. These issues are
			// reminiscent of
			// previously uncovered bugs in older versions of Android's native
			// browser:
			// https://github.com/scottjehl/Device-Bugs/issues/3
			// This fix closes the following bugs ( I use "closes" with
			// reluctance, and stress that
	...
	_close: function( immediate ) {
		...
		// Count down to triggering "popupafterclose" - we have two
		// prerequisites:
	...
	_unenhance: function() {
		...
		this.element
			...
			// Cannot directly insertAfter() - we need to detach() first,
			// because
			// insertAfter() will do nothing if the payload div was not attached
			// to the DOM at the time the widget was created, and so the payload
			// will remain inside the container even after we call
			// insertAfter().
	...
	_closePopup: function( theEvent, data ) {
		...
		if ( theEvent && theEvent.type === "pagebeforechange" && data ) {
			...
			// Determine whether we need to rapid-close the popup, or whether we
			// can take the time
	...
	// TODO No clear deliniation of what should be here and what should be in
	// _open. Seems to be
	...
	open: function( options ) {
		...
		// If history alteration is disabled close on navigate events and leave
		// the url as is
		if ( !( currentOptions.history ) ) {
			self._open( options );
			self._bindContainerClose();

			// When history is disabled we have to grab the data-rel back link
			// clicks so we can
...
// >>group: Forms
// >>description: Select menu extension for menu styling, placeholder options,
// and multi-select.
// >>docs: http://api.jquerymobile.com/selectmenu/
...
return $.widget( "mobile.selectmenu", $.mobile.selectmenu, {
	...
	_handleButtonVclickKeydown: function( event ) {
		...
		if ( event.type === "vclick" ||
			...
			// Do not prevent default, so the navigation may have a chance to
			// actually open the
			// chosen format
		}
	},
	...
	// Focus the button before the page containing the widget replaces the
	// dialog page
	...
	_handleListItemClick: function( event ) {
		...
		// If it's not a multiple select, trigger change after it has finished
		// closing
		...
		// Hide custom select for single selects only - otherwise focus clicked
		// item
		// We need to grab the clicked item the hard way, because the list may
		// have been rebuilt
		if ( this.isMultiple ) {
			this.element.trigger( "change" );
	...
	build: function() {
		...
		// Adding the data-type attribute allows the dialog widget to place the
		// close button before
		...
		// Instantiate the toolbars after everything else so that when they are
		// created they find
	...
	_isRebuildRequired: function() {
		...
		// TODO exceedingly naive method to determine difference ignores value
		// changes etc in favor
	...
	_setTheme: function( key, value ) {
		...
		// We cannot pass inherit to the dialog because pages are supposed to
		// set the theme for
		// the pagecontainer in which they reside. If they set it to inherit the
		// pagecontainer
	...
	_handlePageContainerHide: function( event, data ) {
		if ( data.prevPage[ 0 ] === this.menuPage[ 0 ] ) {
			...
			// After the dialog's done, we may want to trigger change if the
			// value has actually
			// changed
			this._delayedTrigger();
			...
			//
			// TODO extremely confusing dependency on the open method where the
			// pagehide.remove
			// bindings are stripped to prevent the parent page from
			// disappearing. The way we're
			// keeping pages in the DOM right now sucks
			//
			// Rebind the page remove that was unbound in the open function to
			// allow for the parent
			// page removal from actions other than the use of a dialog sized
			// custom select
			//
			// Doing this here provides for the back button on the custom select
			// dialog
	...
	_buildList: function() {
		...
		for ( i = 0; i < numOptions; i++, isPlaceholderItem = false ) {
			...
			// Although using .text() here raises the risk that, when we later
			// paste this into the
			// list item we end up pasting possibly malicious things like
			// <script> tags, that risk
			// only arises if we do something like $( "<li><a href='#'>" + text
			// + "</a></li>" ). We
			// don't do that. We do document.createTextNode( text ) instead,
			// which guarantees that
			// whatever we paste in will end up as text, with characters like <,
			// > and & escaped.
			...
			if ( needPlaceholder &&
				...
				// If we have identified a placeholder, record the fact that it
				// was us who have
				// added the placeholder to the option. Mark it retroactively in
				// the select.
...
/*
 * ! jQuery Mobile Slider Backcompat
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
/*
 * ! jQuery Mobile Textinput Backcompat
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
return $.widget( "mobile.listview", $.mobile.listview, {
	...
	_replaceDividers: function() {
		...
		items.each( $.proxy( function( index, item ) {
			...
			// This tests whether the item is a divider - first we check the
			// class name, and second
			...
			if ( ( item[ 0 ].className && item[ 0 ].className.match( dividerClassRegex ) ) ||
				...
				// If we've found a divider for a heading that already has a
				// divider, remove it to
				...
				if ( existingDivider ) {
					if ( existingDividerText === dividerText ) {
						...
						// We prevent the generation of a divider below by
						// setting the
					...
					// We only keep a reference to an existing divider for one
					// iteration, because
					// the item immediately succeeding an existing divider will
					// inform us as to
					// whether the divider we've found is suitable for the
					// current group
					existingDivider = null;
					existingDividerText = null;
				}

				// If we haven't found a suitable divider and a new group has
				// started, generate a
				// new divider
...
/*
 * ! jQuery Mobile Listview Backcompat
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
/*
 * ! jQuery Mobile Listview Hide Dividers
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
// >>group: Widgets
// >>description: Hides dividers when all items in the section they designate
// become hidden
...
/*
 * ! jQuery Mobile Loader Backcompat
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
if ( $.mobileBackcompat !== false ) {
	$.widget( "mobile.loader", $.mobile.loader, {
		...
		// DEPRECATED as of 1.5.0 and will be removed in 1.6.0 - we no longer
		// support browsers
...
// >>group: Widgets
// >>description: Packcompat Formats groups of links as horizontal navigation
// bars.
// >>docs: http://api.jquerymobile.com/navbar/
// >>demos: http://demos.jquerymobile.com/@VERSION/navbar/
// >>css.structure: ../css/structure/jquery.mobile.navbar.css
...
return $.widget( "mobile.panel", {
		...
		if ( expand || !that.options.positionFixed ) {
			if ( expand ) {
				...
				if ( heightWithMargins < this.document.height() ) {
					...
					// Set the panel's total height (including margins) to the
					// document height
	...
	_bindSwipeEvents: function() {
		...
		// Close the panel on swipe if the swipe event's default is not
		// prevented
		...
		if ( !this._open ) {
			var that = this,
				...
				complete = function() {
					...
					// Bail if the panel was closed before the opening animation
					// has completed
					if ( !that._open ) {
						return;
					}
	...
	close: function( immediate ) {
		if ( this._open ) {
			var that = this,
				...
				// Record what the page is the moment the process of closing
				// begins, because it
...
return $.widget( "mobile.popup", $.mobile.popup, {
	...
	_tryAnArrow: function( p, dir, desired, s, best ) {
		...
		// If the arrow has no wiggle room along the edge of the popup, it
		// cannot
	...
	_getPlacementState: function( clamp ) {
		...
		// The arrow box moves between guideOffset and guideOffset + guideDims -
		// arFull
		state.guideOffset = {
	...
	_placementCoords: function( desired ) {
		...
		// should be placed such that the distance between the tip of the arrow
		// and
		...
		// Could not place the arrow along any of the edges - behave as if
		// showing
	...
	_setOptions: function( opts ) {
		...
		if ( opts.arrow !== undefined ) {
			if ( !ar && opts.arrow ) {
				...
				// Important to return here so we don't set the same options all
				// over
				// again below.
				return;
...
if ( $.mobileBackcompat !== false ) {
	...
	// We override the class being toggled in response to changes in the value
	// of the shadow option
	// because the default implementation toggles the ui-shadow class, whereas
	// we need it to toggle
...
if ( $.mobileBackcompat !== false ) {
	...
	$.widget( "mobile.popup", $.mobile.popup, {
		_setInitialOptions: function() {
			...
			// If the value for the ui-popup-arrow class key has not changed we
			// assume we're
			// dealing with legacy code, so we make sure the presence of the
			// ui-overlay-shadow
			// class in the ui-popup-arrow key reflects the value of the
			// "shadow" option.
		...
		// The presence of the class ui-overlay-shadow in ui-popup must be
		// synchronized to its
		// presence in ui-popup-arrow as long as the shadow option is supported,
		// because the widget
		// backcompat synchronizes its presence in ui-popup to the value of the
		// shadow option.
		_setOption: function( key, value ) {
			var popupHasShadow;
...
return $.widget( "mobile.table", {
	...
	// Expose headers and allHeaders properties on the widget headers references
	// the THs within the
	// first TR in the table
	headers: null,

	// AllHeaders references headers, plus all THs in the thead, which may or
	// may not include
	// several rows
	allHeaders: null,
	...
	// Deprecated as of 1.5.0 and will be removed in 1.6.0 - use refresh()
	// instead
	rebuild: function() {
		this.refresh();
	},
	...
	_refreshHeaderCell: function( cellIndex, element, columnCount ) {
		...
		// Store "cells" data on header as a reference to all cells in the same
		// column as this TH
	...
	_destroy: function() {
		...
		// We have to remove "cells" data even if the table was originally
...
/*
 * ! jQuery Mobile Column-toggling Table
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
// >>group: Widgets
// >>description: Extends the table widget to a column toggle menu and
...
return $.widget( "mobile.table", $.mobile.table, {
	...
	_create: function() {
		...
		// Needed because the superclass calls refresh() which needs to behave
		// differently if
	...
	setColumnVisibility: function( cell, visible ) {
		...
		// Otherwise it's assumed to be a jQuery collection object
			...
			// If it's one of the headers, then we already have the header we
			// wanted
			if ( this.headers.index( cell[ 0 ] ) >= 0 ) {
				header = cell.first();

			// Otherwise we assume it's one of the cells, so look for it in the
			// "cells" data for
			// each header
			} else {
	...
	refresh: function() {
		...
		if ( !this._instantiating && this.options.mode === "columntoggle" ) {
			...
			// Columns not being replaced must be cleared from input
			// toggle-locks
			this._unlock();
			...
			// Make sure columns that were locked before this refresh, and which
			// are still around
...
return $.widget( "mobile.table", $.mobile.table, {
	...
	_updateVariableColumn: function( header, cells, priority, state ) {
		...
		if ( this.options.columnUi || ( state && state.turningOnUI ) ) {
			...
			// Make sure the (new?) checkbox is associated with its header via
			// .jqmData() and that,
	...
	_updateHeaderPriorities: function( state ) {
		...
		if ( this.options.columnUi || state.turningOnUI ) {
			...
			// The controlgroup can only be refreshed after having called the
			// superclass, because
			// the superclass ultimately ends up instantiating the checkboxes
			// inside the
	...
	_removeColumnUi: function( detachOnly ) {
		...
		inputs.each( function() {
			...
			// If we're simply detaching, the checkboxes will be left alone, but
			// the jqmData()
			...
			// The reference from the header to the input has to be removed
			// whether we're merely
	...
	_addColumnUI: function( updater ) {
		...
		// Call the updater before we attach the menu to the DOM, because its
		// job is to populate
		// the menu with checkboxes, and we don't want to do that when it's
		// already attached to
	...
	// Use the .jqmData() stored on the checkboxes to determine which columns
	// have show/hide
	// overrides, and make a list of the indices of those that have such
	// overrides
	_recordLockedColumns: function() {
		var headers = this.headers,
			lockedColumns = [];

		// Find the index of the column header associated with each old checkbox
		// among the
		// post-refresh headers and, if the header is still there, make sure the
		// corresponding
		// column will be hidden if the pre-refresh checkbox indicates that the
		// column is
		...
		this._ui.menu.find( "input" ).each( function() {
			...
			if ( index > -1 ) {
				...
				// The column header associated with /this/ checkbox is still
				// present in the
				// post-refresh table and it is locked, so the column associated
				// with this column
	...
	_restoreLockedColumns: function( lockedColumns ) {
		...
		// At this point all columns are visible, so programmatically
		// check/uncheck all the
		// checkboxes that correspond to columns that were previously unlocked
...
return $.widget( "mobile.table", $.mobile.table, {
	...
	refresh: function() {
		...
		if ( this.options.mode === "reflow" ) {
			...
			// After the refresh completes, we need to iterate over the headers
			// again, but this
			// time in reverse order so that top-level headers are visited last.
			// This causes <b>
			// labels to be added in the correct order using a simple
			// .prepend().
...
/*
 * ! jQuery UI Tabs master http://jqueryui.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
 * license. http://jquery.org/license
 */
...
/*
 * ! jQuery Mobile Tabs Ajax Handling
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
// >>label: Tabs
// >>group: Widgets
// >>description: Extension to make Tabs widget aware of jQuery Mobile's
// navigation
...
/*
 * ! jQuery Mobile Toolbar Backcompat
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
/*
 * ! jQuery Mobile iOS Orientation Fix
 * 
 * @VERSION http://jquerymobile.com
 * 
 * Copyright jQuery Foundation and other contributors Released under the MIT
...
// >>group: Utilities
// >>description: Fixes the orientation change bug in iOS when switching between
...
// This fix addresses an iOS bug, so return early if the UA claims it's
// something else.
var ua = navigator.userAgent,
	zoom,
	evt, x, y, z, aig;
